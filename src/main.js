...
import {createArt} from './art-bg.js?v=20260919-enemies-2';
...
function showOverlay(kind){state=kind;clearInput();$('overlay').hidden=false;$('pills').hidden=kind!=='menu';
 ...
 if(kind==='chapterEnd'){$('eyebrow').textContent='CHAPTER COMPLETE';$('title').innerHTML='الفصل الأول<br><em>مكتمل.</em>';$('description').textContent=`هزمت حارس البوابة. جمعت ${collected} شحنة وأسقطت ${kills} خصوم. استعد — الأسطح تنتظرك.`;$('play').textContent='الفصل الثاني ◀';}
 if(kind==='won'){$('eyebrow').textContent='MISSION COMPLETE';$('title').innerHTML='المدينة<br><em>بأمان.</em>';$('description').textContent=`هزمت قائد الحرس. جمعت ${collected} شحنة وأسقطت ${kills} خصوم. الوقت ${Math.floor(elapsed/60)}:${String(Math.floor(elapsed%60)).padStart(2,'0')} • مرات العودة ${deaths}`;$('play').textContent='العب من جديد ↻';}
...
function tick(dt){if(state!=='playing')return;time+=dt;elapsed+=dt;const controls=input();const previousBottom=player.y+player.h,wasDashing=player.dashTime>0;
 ...
 for(const e of level.enemies){if(e.hp<=0)continue;if(e.windup<=0)e.x+=e.vx*dt;e.hit=Math.max(0,e.hit-dt);if(e.x<e.min){e.x=e.min;e.vx=Math.abs(e.vx);}if(e.x+e.w>e.max){e.x=e.max-e.w;e.vx=-Math.abs(e.vx);}
  if(overlaps(player,e)){if(!e.shield&&player.vy>70&&previousBottom<=e.y+10){e.hp=0;kills++;player.vy=-390;player.coyote=0;burst(e.x+17,e.y+16,'#e0b164',12);sound(280,.12,'triangle');}else hurt();}
 }
 for(const event of stepCombat(level,player,enemyShots,dt,(e,isBoss)=>art.enemyMuzzle(e,time,isBoss))){if(event==='hit')hurt();if(event==='boss')toast(`${BOSS_NAMES[chapter]} — تفادَ النيران بالاندفاع!`);}
 ...
 for(const b of bullets){b.x+=b.vx*dt;b.life-=dt;if(b.life<=0)continue;
  for(const e of [...level.enemies,level.boss]){if(e.hp<=0||!overlaps(b,bulletTargetBounds(e)))continue;
   if(e.shield){const center=e.x+e.w/2,facing=e.windup>0&&e.aimX?e.aimX<center?-1:1:e.vx<0?-1:1;if(b.vx*facing<0){b.life=0;burst(b.x,b.y,'#8fa5b0',4);sound(320,.06,'square',.03);break;}}
   b.life=0;e.hp--;e.hit=.12;burst(b.x,b.y,'#e6c878',5);if(e.hp<=0){kills++;burst(e.x+17,e.y+16,'#dc844e',e===level.boss?45:14);if(e===level.boss){enemyShots=[];toast(chapter<CHAPTER_COUNT?'سقط الحارس — توجّه إلى نقطة الإخلاء!':'سقط قائد الحرس — توجّه إلى نقطة الإخلاء!');sound(100,.5,'sawtooth',.058);}}break;}
 }
